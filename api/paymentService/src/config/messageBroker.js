...
const connect = async () => {
  try {
    ...
    const queue = await channel.assertQueue('paymentservice_events', { durable: true });
    await channel.bindQueue(queue.queue, 'beleza_events', 'booking.*');
    console.log('[Payment Service] RabbitMQ OK');
    ...
    console.error('[Payment Service] RabbitMQ:', err);
  }
};
